      ...
      if(trigger.key_code != 0) {
          ...
          foreach(uint lock_modifier in lock_modifiers) {
              if (trigger.with_mouse) {
                  display.grab_button(
                    trigger.key_code,
                    trigger.modifiers|lock_modifier,
                    xid, false,
                  ...
                  display.grab_key(
                    trigger.key_code, trigger.modifiers|lock_modifier,
          ...
          if(id == binding.id) {
              foreach(uint lock_modifier in lock_modifiers) {
                  if (binding.trigger.with_mouse) {
                      display.ungrab_button(
                        binding.trigger.key_code,
                        binding.trigger.modifiers|lock_modifier, xid
                      );
                  } else {
                      display.ungrab_key(
                        binding.trigger.key_code,
