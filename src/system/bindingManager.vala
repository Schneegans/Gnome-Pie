...
// A class storing global key strokes. When one of them is pressed,
// a signal is emitted.

public class BindingManager : GLib.Object {

    // this signal is emitted when a stored stroke is pressed
    public static signal on_press(string id);
    
    // stores bindings, which are invoked even if Gnome-Pie doesn't have the current focus
    private static Gee.List<Keybinding> _global_bindings = new Gee.ArrayList<Keybinding>();
    
    // stores bindings, which are only activated when Gnome-Pie has focus
    private static Gee.List<Keybinding> _local_bindings = new Gee.ArrayList<Keybinding>();
    ...
    private class Keybinding {
        ...
        public Keybinding(string accelerator, int keycode, Gdk.ModifierType modifiers, bool down, string id) {
    ...
    // public interface
    public void bind_global_press(string stroke, string id) {
        bind_global(stroke, true, id);
    }
    
    public void bind_local_press(string stroke, string id) {
        bind_local(stroke, true, id);
    }
    
    public void bind_global_release(string stroke, string id) {
        bind_global(stroke, false, id);
    }
    
    public void bind_local_release(string stroke, string id) {
        bind_local(stroke, false, id);
    }
    ...
    // private stuff
    private void bind_global(string accelerator, bool down, string id) {
        ...
        if(keycode != 0) {
            ...
            Keybinding binding = new Keybinding(accelerator, keycode, modifiers, down, id);
            _global_bindings.add(binding);
        }
    }
    
    private void bind_local(string accelerator, bool down, string id) {
